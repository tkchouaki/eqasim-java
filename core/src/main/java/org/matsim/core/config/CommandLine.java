...
public class CommandLine {
    ...
    public void applyConfiguration(Config config) throws ConfigurationException {
        this.applyConfiguration(config, new ArrayList<>());
    }

    public void applyConfiguration(Config config, Collection<ConfigGroup> configGroupList) throws ConfigurationException {
        Map<String, ConfigGroup> configGroupMap = new HashMap<>();
        for(ConfigGroup configGroup: configGroupList) {
            configGroupMap.put(configGroup.getName(), configGroup);
        }
        List<String> configOptions = options.keySet().stream().filter(o -> o.startsWith(CONFIG_PREFIX + ":"))
                .collect(Collectors.toList());
        for (String option : configOptions) {
            processConfigOption(config, option, option.substring(CONFIG_PREFIX.length() + 1), configGroupMap);
        }
    }
    ...
    private void processConfigOption(Config config, String option, String remainder, Map<String, ConfigGroup> configGroupMap) throws ConfigurationException {
        ...
        if (separatorIndex > -1) {
            ...
            ConfigGroup moduleConfigGroup;
            if(config.getModules().containsKey(module)) {
                moduleConfigGroup = config.getModules().get(module);
            } else if (configGroupMap.containsKey(module)) {
                moduleConfigGroup = configGroupMap.get(module);
                config.addModule(moduleConfigGroup);
            } else {
                throw new ConfigurationException(
                        String.format("Invalid MATSim option: '%s'. Module '%s' does not exist in the config and no fallback is given", remainder, module));
            }
            processParameter(option, module, moduleConfigGroup, newRemainder);
        } else {
            throw new ConfigurationException(
    ...
    private ConfigGroup findChild(ConfigGroup configGroup, String childSelector) {
        ConfigGroup selectedChild = null;
        if(childSelector.contains("[")) {
            int selectorStartIndex = childSelector.indexOf('[');
            int selectorEndIndex = childSelector.indexOf(']');
            int equalIndex = childSelector.indexOf('=');

            if (selectorStartIndex > -1 && selectorEndIndex > -1 && equalIndex > -1) {
                if (selectorStartIndex < equalIndex && equalIndex < selectorEndIndex) {
                    String parameterSetType = childSelector.substring(0, selectorStartIndex);
                    String selectionParameter = childSelector.substring(selectorStartIndex + 1, equalIndex);
                    String selectionValue = childSelector.substring(equalIndex + 1, selectorEndIndex);
            ...
            Collection<? extends ConfigGroup> parameterSets = configGroup
                    .getParameterSets(childSelector);
            if(parameterSets.size() == 0) {
                selectedChild = configGroup.createParameterSet(childSelector);
            }
            else {
                for(ConfigGroup parameterSet: parameterSets) {
    ...
    private void processParameter(String option, String path, ConfigGroup configGroup, String remainder)
        ...
        if(separatorIndex > -1) {
            ...
            String parameterSetType = matcher.group(1);
            String parameterSetCriteria = matcher.group(3);
            String parameterSetCriteriaValue = matcher.group(4);
            boolean addIfMissing = matcher.group(5) != null;
            ...
            for(ConfigGroup parameterSet: configGroup.getParameterSets(parameterSetType)) {
                ...
                if(parameterSetCriteria == null || parameterSet.getParams().get(parameterSetCriteria).equals(parameterSetCriteriaValue)) {
                    if(selectedParameterSet != null) {
                        throw new ConfigurationException(String.format("More than one candidate found for %s in %s", childSelector, path));
                    }
                    selectedParameterSet = parameterSet;
                }
            }
            if(selectedParameterSet == null && addIfMissing) {
                selectedParameterSet = configGroup.createParameterSet(parameterSetType);
                if(parameterSetCriteria != null) {
                    if(!selectedParameterSet.getParams().containsKey(parameterSetCriteria)) {
                        throw new ConfigurationException(String.format("Parameter %s not found in %s", parameterSetCriteria, path));
                    }
                    selectedParameterSet.addParam(parameterSetCriteria, parameterSetCriteriaValue);
                }
                configGroup.addParameterSet(selectedParameterSet);
            } else if(selectedParameterSet == null) {
                throw new ConfigurationException(String.format("No parameter set matching %s found in %s", childSelector, path));
            }
            String newPath = String.format("%s.%s", path, childSelector);
            String newRemainder = remainder.substring(separatorIndex+1);
            this.processParameter(option, newPath, selectedParameterSet, newRemainder);
        } else {
