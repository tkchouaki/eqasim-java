...
public class EventFiringTourBasedModel implements DiscreteModeChoiceModel{
    ...
    final private DiscreteModeChoiceModel.FallbackBehaviour fallbackBehaviour;
    ...
    public EventFiringTourBasedModel(TourEstimator estimator, ModeAvailability modeAvailability,
                                     ...
                                     DiscreteModeChoiceModel.FallbackBehaviour fallbackBehaviour, TimeInterpretation timeInterpreterFactory,
    ...
    public List<TripCandidate> chooseModes(Person person, List<DiscreteModeChoiceTrip> trips, Random random)
            throws DiscreteModeChoiceModel.NoFeasibleChoiceException {
            ...
            if (tourFilter.filter(person, tourTrips)) {
                ...
                if (!selectedCandidate.isPresent()) {
                    switch (fallbackBehaviour) {
                        ...
                        case EXCEPTION:
                            throw new DiscreteModeChoiceModel.NoFeasibleChoiceException(buildFallbackMessage(tripIndex, person, ""));
                    }
                }
                this.eventsManager.processEvent(new TourSelectorEvent(-1, person, utilityCandidates, (TourCandidate) selectedCandidate.get()));
