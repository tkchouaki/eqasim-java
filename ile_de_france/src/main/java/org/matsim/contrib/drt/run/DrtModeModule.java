...
import org.matsim.contrib.drt.optimizer.rebalancing.remoteRebalancing.DrtModeRemoteRebalancingModule;
import org.matsim.contrib.drt.optimizer.rebalancing.remoteRebalancing.RemoteRebalancingParams;
...
public final class DrtModeModule extends AbstractDvrpModeModule {
	...
	public void install() {
		...
		if (drtCfg.getRebalancingParams().isPresent()) {
			...
			} else if (rebalancingParams.getRebalancingStrategyParams() instanceof RemoteRebalancingParams) {
				install(new DrtModeRemoteRebalancingModule(drtCfg));
			} else {
